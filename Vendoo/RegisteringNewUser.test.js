...
describe ('Dev Vendoo form registering', () => {
    ...
    it('Error message for used email', () => {
        cy.contains('The email address is already in use by another account.').should('not.exist')

        cy.get('input[name="email"]').clear().type('fake@gmail.com')
        cy.get('button[type="submit"]').click()

        cy.contains('The email address is already in use by another account.', {timeout: 15000}).should('exist')
    ...
    it('Registering account and making sure to land on inventory after clicking Get Started', () => {
        ...
        cy.url({timeout : 30000}).should('eq', 'https://dev.vendoo.co/app/onboarding')
    ...
    it('Logging Out', () => {
        cy.get('a[href="/app/#"]', {timeout: 25000}).click({force: true})
    })
